...
**Project:** gt-sandbox (product name TBD — see §10) · **Version:** v0.5.3 · **Updated:** 2026-07-19
...
## Current handoff snapshot — S16, 2026-07-19 (P4 committed locally; P0–P4 awaiting review/push)

**Current state (fact).** Planning remains owner-ratified for the v1 build. P0’s complete walking skeleton remains in four local commits: static Next.js App Router routes, visual shell and tokens, package manifest/lockfile, Vitest, Playwright, lint/format tooling, invariant and bundle scripts, CI workflow, deployment CSP, and portable-host header documentation. P1’s exact, framework-free core remains in four further local commits: bigint rational arithmetic, normal-form types, the six-game catalog and hand-authored pure-analysis oracles, pure Nash/dominance/Pareto/equilibrium-selection/classifier solvers, an independent pure-profile verifier, and the seeded RNG. P2 is complete in three further local commits (`v0.3.0`–`v0.3.2`): the static PD route contains a deterministic ten-round price-war session against Saint, Cynic, or Copycat; engine-derived payoffs, dominance, and pure-NE indicators; live outcome narration; a semantic matrix and history; reduced-motion parity; and reducer-enforced loop/focus behavior. P3 is complete in four further local commits (`v0.4.0`–`v0.4.3`): exact equal-size support enumeration through 4×4, an independent closed-form 2×2 path, a mixed-equilibrium verifier, formal exact degeneracy witnesses, catalog/test coverage, and a 39-case external Gambit v16.6.0 fixture corpus. Fixture containers have no network or host mounts, use a read-only root filesystem, run as non-root with all Linux capabilities dropped, and exchange only generated game text through standard input/output. P4 is complete in four further local commits (`v0.5.0`–`v0.5.3`): exact signed fixed-action hindsight and mix-vs-Nash statistics, a shared progressive six-panel Analysis drawer, keyboard-operable glossary definitions, a full engine-derived PD reveal, and exported-browser drawer acceptance. P0–P4 remain unpushed and await owner review; there is still no deployment or live URL.

**Repository state (at the S16 handoff).** Branch `main` tracks `origin/main`; local `main` is nineteen commits ahead: the four P0 commits, four P1 commits, three P2 commits, four P3 commits, and four P4 commits `v0.5.0`–`v0.5.3` recorded in §11. The working tree is clean; no historical commit was amended, pushed, or tagged. `.DS_Store`, generated coverage, and the local pnpm store remain ignored.

**Version state (fact).** `v0.5.3` is the local P4 handoff version and agrees across `package.json`, README, this file, and CHANGELOG. `pnpm-lock.yaml` has no root package-version field, so the version-only P4 commits do not modify it. The next version is owner-directed.
...
| -------- | ------------------------------------ | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| 1        | P0–P4 review and sign-off (§6)       | Owner review                        | Review the nineteen local commits, including the four-commit P4 series; authorize a push.                          |
...
| 3        | P5–P9, strictly in phase order       | P4 accepted                         | Review each phase; explicitly authorize every commit and push.                                                     |
...
- **Verified implementation bugs:** none observed in P0’s static shell, P1’s exact core, P2’s PD session, P3’s support-enumeration/degeneracy/external cross-oracle suites, or P4’s reveal/statistics layer.
...
- Do not claim planned capabilities are already implemented: P0, P1’s pure exact core, P2’s PD session, P3’s mixed-equilibrium/oracle phase, and P4’s reveal layer are implemented in local commits. P5 onward remains unimplemented.
...
1. **Review P0–P4** — P0 is locally committed in the four commits recorded at S6; P1 in the four commits recorded at S8–S9; P2 in the three commits recorded at S11; P3 in the four commits recorded at S14; and P4 in the four commits recorded at S16. `CI=true pnpm verify:full` is green. Nothing is pushed.
